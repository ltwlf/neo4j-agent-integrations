...
Defaults connect to the public `companies` Neo4j demo graph and provision a Microsoft Foundry account, project, `gpt-4o-mini` model deployment, and a Foundry User role assignment on the project for you. `deploy.sh` writes a shared `microsoft-foundry/.env` that every example sources.
